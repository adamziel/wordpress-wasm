...
class WP_Git_Pack_Processor {
	...
	public static function deflate( string $content ): string {
		$context = deflate_init( ZLIB_ENCODING_DEFLATE, array( 'level' => 9 ) );
	...
	public static function object_header( int $type, int $size ): string {
	 ...
	 * }
	 */
	public static function encode_tree_bytes( $tree ) {
		$tree_bytes = '';
	...
	public static function create_object( $basic_data ) {
		$object = array(
			'ofs' => 0,
	...
	private static function wrap_object( $type, $object ) {
		$length    = strlen( $object );
		$type_name = WP_Git_Pack_Processor::OBJECT_NAMES[ $type ];
		return "$type_name $length\x00" . $object;
	}

	public static function encode_packet_lines( array $payloads ): string {
		$lines = array();
	...
	public static function encode_packet_line( string $payload, $channel = '' ): string {
	 ...
	 */
	public static function decode( $pack_bytes, $pack_index = null ) {
		if ( null === $pack_index ) {
	...
	private static function wrap_git_object( $type, $object ) {
		$length    = strlen( $object );
		$type_name = self::OBJECT_NAMES[ $type ];
		return "$type_name $length\x00" . $object;
	}

	private static function applyDelta( $base_bytes, $delta_bytes ) {
		$offset = 0;
		...
		while ( $offset < strlen( $delta_bytes ) ) {
			...
			if ( $byte & 0x80 ) {
				...
				$copySize   = 0;
				if ( $byte & 0x01 ) {
					$copyOffset |= ord( $delta_bytes[ $offset++ ] );
				}
				if ( $byte & 0x02 ) {
					$copyOffset |= ord( $delta_bytes[ $offset++ ] ) << 8;
				}
				if ( $byte & 0x04 ) {
					$copyOffset |= ord( $delta_bytes[ $offset++ ] ) << 16;
				}
				if ( $byte & 0x08 ) {
					$copyOffset |= ord( $delta_bytes[ $offset++ ] ) << 24;
				}
				if ( $byte & 0x10 ) {
					$copySize |= ord( $delta_bytes[ $offset++ ] );
				}
				if ( $byte & 0x20 ) {
					$copySize |= ord( $delta_bytes[ $offset++ ] ) << 8;
				}
				if ( $byte & 0x40 ) {
					$copySize |= ord( $delta_bytes[ $offset++ ] ) << 16;
				}
				if ( $copySize === 0 ) {
					$copySize = 0x10000;
				}
	...
	public static function parse_commit_body( $commit_message ) {
		$lines  = explode( "\n", $commit_message );
		$parsed = array();
	...
	public static function parse_tree_bytes( $treeContent ) {
		$offset = 0;
		$files  = array();
	...
	private static function readVariableLength( $data, &$offset ) {
		$result = 0;
		$shift  = 0;
		do {
	...
	public static function parse_pack_data( $packData, $pack_index = null ) {
		$offset = 0;
		...
		while ( $resolved_objects < count( $objects ) ) {
			...
			for ( $i = 0; $i < count( $objects ); $i++ ) {
				...
				} elseif ( $objects[ $i ]['type'] === self::OBJECT_TYPE_REF_DELTA ) {
	 ...
	 */
	private static function inflate_object( string $packData, int &$offset, int $uncompressedSize ): ?string {
	...
	private static function parse_pack_header( string $packData, int &$offset ) {
