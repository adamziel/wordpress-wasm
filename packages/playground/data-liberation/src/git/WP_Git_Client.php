...
class WP_Git_Client {
	...
	public function __construct( WP_Git_Repository $index, $options = array() ) {
	...
	public function force_push_one_commit() {
		$push_ref_name = $this->index->get_ref_head( 'HEAD', array( 'resolve_ref' => false ) );
	...
	public function force_pull( $branch_name = null, $path = '/' ) {
		if ( ! $branch_name ) {
			$branch_name = $this->index->get_ref_head( 'HEAD', array( 'resolve_ref' => false ) );
		...
		$new_oids         = $remote_index->find_objects_added_in(
			$remote_head,
			$local_ref ?: null,
			array(
	...
	public static function accumulate_pack_data_from_multiplexed_chunks( $raw_response ) {
		$parsed_pack_data = array();
	...
	public static function parse_multiplexed_pack_data( $bytes ) {
		$offset = 0;
		while ( $offset < strlen( $bytes ) ) {
