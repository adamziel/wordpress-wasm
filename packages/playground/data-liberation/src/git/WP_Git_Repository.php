...
class WP_Git_Repository {
	...
	public function find_path_descendants( $path ) {
		...
		while ( ! empty( $stack ) ) {
			...
			} elseif ( $this->get_type() === WP_Git_Pack_Processor::OBJECT_TYPE_BLOB ) {
				$oids[] = $this->get_oid();
			}
		}
		return $oids;
	...
	public function find_objects_added_in( $new_commit_hash, $old_commit_hash = WP_Git_Repository::NULL_OID, $options = array() ) {
	...
	public function commit( $options = array() ) {
		...
		foreach ( $updates as $path => $content ) {
			...
			$changed_trees[ dirname( $path ) ]['entries'][ basename( $path ) ] = array(
				'name' => basename( $path ),
		...
		foreach ( $move_trees as $old_path => $new_path ) {
			...
			$changed_trees[ dirname( $new_path ) ]['entries'][ basename( $new_path ) ] = array(
		...
		// Update HEAD
		$head_ref = $this->get_ref_head( 'HEAD', array( 'resolve_ref' => false ) );
	...
	public function squash( $squash_into_commit_oid, $squash_until_ancestor_oid ) {
		...
		$current_branch = $this->get_ref_head( 'HEAD', array( 'resolve_ref' => false ) );
	...
	private function mark_tree_path_changed( &$changed_trees, $path ) {
		while ( $path !== '/' ) {
			if ( ! isset( $changed_trees[ $path ] ) ) {
				$changed_trees[ $path ] = array( 'entries' => array() );
			}
			$path = dirname( $path );
		}
	}
	...
	private function commit_tree( $path, $changed_trees ) {
		...
		foreach ( $changed_trees as $child_path => $child_tree ) {
			if ( dirname( $child_path ) === $path && $child_path !== '/' ) {
				...
				$tree_objects[ basename( $child_path ) ] = array(
