...
test('HTTPS requests via file_get_contents() should work (2)', async ({
	website,
	wordpress,
}) => {
	const blueprint: Blueprint = {
	...
	await website.goto(`/#${JSON.stringify(blueprint)}`);
	await expect(wordpress.locator('body'))
		.toContainText(`string(13061) "# WordPress Playground and PHP WASM (WebAssembly)
